...
import { MonthlyTrendData } from "@/services/inventoryTurnover";
...
// export default function TrendTurnoverChartMonthly({ data, year }: Props) {
...
//       <CardHeader>
//         <CardTitle className="text-lg font-bold text-black">Monthly Overall Inventory Turnover Trend</CardTitle>
//       </CardHeader>
...
//                 if (value === undefined) return ["0.0", labelName ?? "Inventory Turnover"];
...
export default function TrendTurnoverChartMonthly({ data, year }: Props) {
  ...
  const chartData = data.map(item => ({
    ...item,
    targetLower: year < 2026 ? 2.5 : 3.0,
    targetUpper: year < 2026 ? 4.0 : 3.8,
  }));

  // Hitung nilai maksimum secara dinamis berdasarkan data tipe yang sedang dipilih dan Target
  const yAxisMax = useMemo(() => {
    const targetUpper = year < 2026 ? 4.0 : 3.8;
    let maxVal = targetUpper;

    const activeKey = 
      selectedType === "overall" ? "overallTurnover" : 
    ...
    <>
      ...
      <Card ref={cardRef} className="shadow-sm rounded-xl overflow-visible border-none">
        <CardHeader className="flex flex-row items-center justify-between gap-2 px-4 sm:px-6">
          <CardTitle className="text-sm sm:text-lg font-bold text-black line-clamp-2">{`Monthly ${selectedType.charAt(0).toUpperCase() + selectedType.slice(1)} Inventory Turnover Trend`}</CardTitle>
      ...
      <CardContent className="h-[350px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <defs>
              <linearGradient id="colorITO" x1="0" y1="0" x2="0" y2="1">
            ...
            <Tooltip 
              ...
              contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
              itemSorter={(item) => {
                if (item.dataKey === 'targetUpper') return -1;               
                if (item.dataKey === 'targetLower') return 1;
                return 0; // Data Utama (yang bukan target) akan berada di posisi 0 (tengah)
              }}
              formatter={(value: number | string | undefined, labelName: any) => {
                if (value === null || value === undefined) return ["-", labelName ?? "Value"];
            ...
            <Legend 
              ...
              iconType="circle"
              iconSize={8}
              wrapperStyle={{ fontSize: '10px', paddingTop: '20px', position: 'relative' }} 
            />
            ...
            <Area 
              ...
              dot={(props: any) => {
                ...
                const activeKey = selectedType === "overall" ? "overallTurnover" : selectedType === "fish" ? "overallFish" : "overallShrimp";
              ...
              fill="url(#colorITO)" 
              strokeWidth={2} 
              name={`${selectedType.charAt(0).toUpperCase() + selectedType.slice(1)} Inventory Turnover`}
            />              
